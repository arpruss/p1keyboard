	...
	protected void parseDPad(byte[] data, int offset) {
		...
		for(int i = 0; i < 4; i++) {
			if ((v & 1) != m_dpad[i]) {
				...
				keypressBroadcast.putExtra(P1Service.EVENT_KEYPRESS_ACTION, m_dpad[i] == 1 ? KeyEvent.ACTION_DOWN : KeyEvent.ACTION_UP);
				keypressBroadcast.putExtra(P1Service.EVENT_KEYPRESS_KEY, DPAD[i]);
