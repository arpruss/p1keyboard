	...
	private synchronized void connectToDevice(String address, String driver, String sessionId, boolean startnotification) {
		try {
			...
			synchronized (m_readers) {
				...
				else if (driver.toLowerCase().equals(PalmOneWirelessKeyboardReader.DRIVER_NAME.toLowerCase()))
					reader = new PalmOneWirelessKeyboardReader(address, sessionId, getApplicationContext(), startnotification);
